...
class DataIterator:
    def __init__(self, data_dir, test=False):
        ...
        length = len(self.image1_names)
        len1 = length * 0.1  # 1:9
        self.imageval_names = self.image1_names[:int(len1)]
        self.imagetrain_names = self.image1_names[int(len1):]
        ...
        if test == False:
            ...
            self.train_labels = [i for i in range(len(self.imagetrain_names))]
            self.val_labels = [i for i in range(len(self.imageval_names))]
            self.image_labels = [i for i in range(len(self.image1_names))]

            self.train_names = [str(file_name[len(data_dir):].split(os.sep)[0]) for file_name in self.imagetrain_names]
            self.val_names = [str(file_name[len(data_dir):].split(os.sep)[0]) for file_name in self.imageval_names]
    ...
    def input_pipeline(self, batch_size, num_epochs=None, aug=False, m="all"):
        if m == "train":
...
def netmodel(keep_prob, images, labels):
    ...
    max_pool_2 = slim.max_pool2d(norm3, [2, 2], [2, 2], padding='SAME')
    ...
    max_pool_3 = slim.max_pool2d(norm5, [2, 2], [2, 2], padding='SAME')
    ...
    max_pool_4 = slim.max_pool2d(norm8, [2, 2], [2, 2], padding='SAME')
    ...
    max_pool_5 = slim.max_pool2d(norm11, [2, 2], [2, 2], padding='SAME')
