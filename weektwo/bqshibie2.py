...
tf.app.flags.DEFINE_string('checkpoint_dir', './checkpoint5/', 'the checkpoint dir')
...
def build_graph(top_k):
    ...
    def has_pos():
        return tf.reduce_sum(loss * pos_weight) / n_pos

    def has_neg():
        return tf.reduce_sum(loss * neg_weight) / n_neg

    def no():
        return tf.constant(0.0)

    '''has_pos=tf.reduce_sum(loss * pos_weight) / n_pos
    has_neg=tf.reduce_sum(loss * neg_weight) / n_neg
    no = tf.constant(0.0)'''
    pos_loss = tf.cond(n_pos > 0, has_pos, no)
    neg_loss = tf.cond(n_neg > 0, has_neg, no)
    sensitive_loss = (pos_loss + neg_loss) / 2.0
...
def validation():  # 验证生成 json文件
    ...
    result_file = 'week2_submission4.json'
