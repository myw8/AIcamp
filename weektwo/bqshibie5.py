...
tf.app.flags.DEFINE_string('checkpoint_dir', './checkpoint11/', 'the checkpoint dir')
...
tf.app.flags.DEFINE_string('log_dir', './log3', 'the logging dir')
...
def build_graph(top_k):
    ...
    ce_loss = tf.losses.get_total_loss()'''
    ...
    num_examples = tf.reduce_prod(tf.shape(labels))
    ...
    sensitive_loss = tf.reduce_sum(ce_loss * loss_weight) / tf.reduce_sum(loss_weight)
    loss=sensitive_loss
...
def train():  # 训练
    ...
    with tf.Session(config=config) as sess:
        train_images, train_labels,train_names = data_feeder.input_pipeline(batch_size=FLAGS.batch_size, aug=False,
                                                                             m="all")
...
def validation():  # 验证生成 json文件
    ...
    result_file = 'week2_submission5.json'
